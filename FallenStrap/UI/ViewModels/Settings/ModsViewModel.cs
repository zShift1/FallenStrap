    ...
    public class ModsViewModel : NotifyPropertyChangedViewModel
    {
        ...
        private void ManageCustomCursor()
        {
            ...
            else
            {
                var dialog = new OpenFileDialog
                {
                    Filter = "Imagen de cursor (*.png)|*.png"
                };

                if (dialog.ShowDialog() != true)
                    return;

                byte[] pngHeader = new byte[4] { 0x89, 0x50, 0x4E, 0x47 };

                if (!File.ReadAllBytes(dialog.FileName).Take(4).SequenceEqual(pngHeader))
                {
                    Frontend.ShowMessageBox("El archivo no es una imagen PNG válida.", MessageBoxImage.Error);
                    return;
                }
